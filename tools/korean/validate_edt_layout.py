...
The Korean patch is sourced from older JA2/1.13 data, where some dialogue files
contain appended records.  Imported files must therefore be trimmed to the
exact vanilla/Stracciatella byte length.  This validator checks filename
coverage, fixed-record alignment, reference-size parity, and decode-safe UTF-16
storage for the three runtime EDT families used by the Korean mod.
"""
...
EXPECTED_COUNTS = {
    "MercEdt": 70,
    # Original 161-file import missed 097 (Skyrider), 138 (Waldo), and 159
    # (Speck).  The non-zero-padded reference-only 56.EDT is not generated by
    # GetDialogueTextFilename(), which always formats profile IDs as 3 digits.
    "NPCData": 164,
    "BinaryData": 16,
}
...
# One bundled Chinese reference file is a legacy non-zero-padded duplicate.
# Current dialogue lookup uses "{03d}.edt", so it can never request 56.EDT.
    ...
    for row in range(records):
        ...
        for column, width in enumerate(layout):
            ...
            try:
                ...
                # Runtime forcibly terminates the final code unit.  Full-width
                # reference fields are legal legacy data, so do not fail here.
                end = width - 1
...
def validate_group(data_root: Path, ref_root: Path, group: str) -> tuple[int, int, list[str]]:
    ...
    if len(files) != EXPECTED_COUNTS[group]:
        errors.append(f"{directory}: expected {EXPECTED_COUNTS[group]} EDT files, found {len(files)}")
