...
# Keep these public helpers in this module because the decoded-text audit shares
# the exact same classic EDT layout/decryption rules.
REFERENCE_DIRS = {
    "MercEdt": "MercEdt",
    "NPCData": "npcdata",
...
def decrypt_rot1(raw: int) -> int:
    """Mirror LoadEncryptedData's ROT-1 transform for classic EDT UTF-16 units."""
    return raw - 1 if raw > 33 else raw


def row_bytes(layout: tuple[int, ...]) -> int:
    return sum(layout) * BYTES_PER_CHAR
...
def reference_dir(root: Path, group: str) -> Path:
    return root / REFERENCE_DIRS[group]


def malformed_decoded_unit(raw: int) -> bool:
    unit = decrypt_rot1(raw)
    return (
        0xD800 <= unit <= 0xDFFF
        or 0xFDD0 <= unit <= 0xFDEF
        or unit in (0xFFFE, 0xFFFF)
    )
...
def validate_payload(path: Path, layout: tuple[int, ...]) -> list[str]:
    ...
    stride = row_bytes(layout)
    if not data:
        return [f"{path}: empty EDT file"]
    if len(data) % stride:
        return [f"{path}: {len(data)} bytes is not divisible by {stride}-byte record size"]
        ...
        for column, width in enumerate(layout):
            ...
            for idx, raw in enumerate(units[:end]):
                if malformed_decoded_unit(raw):
                    return [
                        f"{path}: row {row} col {column} char {idx} has decode-unsafe UTF-16 unit"
                    ]
...
def validate_group(
    data_root: Path, ref_root: Path, group: str
    ...
    if len(files) != EXPECTED_COUNTS[group]:
        errors.append(
            f"{directory}: expected {EXPECTED_COUNTS[group]} EDT files, found {len(files)}"
        )
    ...
    if group == "MercEdt":
        ...
        if missing:
            errors.append(f"{directory}: missing MercEdt files: {', '.join(missing)}")
        if extra:
            errors.append(f"{directory}: unexpected MercEdt files: {', '.join(extra)}")
    elif group == "BinaryData":
        expected = {x.casefold() for x in BINARY_LAYOUTS}
        missing, extra = sorted(expected - actual), sorted(actual - expected)
        if missing:
            errors.append(f"{directory}: missing BinaryData files: {', '.join(missing)}")
        if extra:
            errors.append(f"{directory}: unexpected BinaryData files: {', '.join(extra)}")
    else:
        expected = {
            p.name.casefold()
            for p in refs
            if layout_for("NPCData", p.name) is not None
            and p.name.casefold() not in REFERENCE_ONLY_NPC_FILES
        }
        missing, extra = sorted(expected - actual), sorted(actual - expected)
        if missing:
            errors.append(f"{directory}: missing vanilla NPCData files: {', '.join(missing)}")
        if extra:
            errors.append(f"{directory}: unexpected NPCData files: {', '.join(extra)}")

    total_bytes = 0
    for path in files:
        layout = layout_for(group, path.name)
        if layout is None:
            errors.append(f"{path}: unknown EDT layout")
            continue
        total_bytes += path.stat().st_size
        ref = ref_index.get(path.name.casefold())
        if ref is None:
            errors.append(f"{path}: no same-named bundled reference")
            continue
        if path.stat().st_size != ref.stat().st_size:
            errors.append(
                f"{path}: size mismatch korean={path.stat().st_size}, reference={ref.stat().st_size}"
            )
            continue
